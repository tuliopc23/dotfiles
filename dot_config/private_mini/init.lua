...
-- User Command: Update MiniMax and re-apply custom patches
vim.api.nvim_create_user_command('UpdateMiniMax', function()
  local script = vim.fn.stdpath('config') .. '/scripts/update.lua'
  -- Run in a terminal buffer so you can see the output
  vim.cmd('terminal nvim -l ' .. script)
end, { desc = 'Update MiniMax and patch custom config' })
