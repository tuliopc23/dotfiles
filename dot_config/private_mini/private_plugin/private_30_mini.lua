...
now(function() vim.cmd('colorscheme xcode-doom') end)
  ...
  require('mini.statusline').setup({
    ...
    content = {
      active = function()
        ...
        local icon = mode_map[vim_mode] or ' '
        
        -- Sections
        local git           = MiniStatusline.section_git({ trunc_width = 40 })
        local diff          = MiniStatusline.section_diff({ trunc_width = 75 })
        local diagnostics   = MiniStatusline.section_diagnostics({ trunc_width = 75 })
        -- Smart filename: "Dashboard" for starter, icon+name for others
        ...
        return MiniStatusline.combine_groups({
          { hl = mode_hl,                  strings = { ' ' .. icon .. ' ' } }, -- Icon only, bold background
          { hl = 'MiniStatuslineDevinfo',  strings = { git, diff } },
          '%<', -- Mark general truncate point
          { hl = 'MiniStatuslineFilename', strings = { filename } },
          '%=', -- End left alignment
          { hl = 'MiniStatuslineUrgent',   strings = { macro } },
...
later(function()
  ...
  -- Custom picker for zoxide
  pick.registry.zoxide = function(local_opts)
    ...
    local_opts = vim.tbl_deep_extend('force', {
      source = {
        ...
        choose = function(item)
          -- Change directory and open explorer
          vim.fn.chdir(item)
          MiniFiles.open()
        end,
      },
